...
- No `npm install` — Deno resolves dependencies from `deno.json` imports on
  first run
...
Use `POST /dev/login` to script multiple players in one session for local
testing.
...
3. In browser console or curl: `POST /dev/login { "name": "Alice" }` → sets
   `playerId` cookie
