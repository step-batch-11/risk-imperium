...
Decompose the agreed plan into technical tasks, each with clear acceptance
criteria. Do not exit this skill until the user has confirmed the task list.
Save the result at `docs/<story_number>/tasks.md`.
...
If the plan file is missing or was never agreed to, return to the
`story-planning` skill first.
...
- **Independently implementable** — each can be coded, tested, and committed on
  its own
...
**Description** What needs to be done, referencing the relevant section of the
plan.
...
> "While breaking down the tasks, I found a gap: [describe gap]. I recommend we
> [proposed resolution]. How would you like to proceed?
>
...
Do not silently paper over gaps with assumptions unless the user explicitly
chooses option 2.
...
> "Here are the N tasks I've defined. Does this breakdown look complete? Any
> tasks to add, remove, or split before we start implementing?"
...
Signal to the orchestrator: **task creation complete, return to
...
**Too many tasks (8+)** Surface this to the user:

> "This breakdown has N tasks, which may span multiple sessions. Would you like
> to prioritise a subset for this session, or proceed with the full list?"

**Circular dependencies** If two tasks depend on each other, flag it immediately
and propose a split or merge.

**Task requires a technology not currently in the stack** Flag it as a decision
point before writing the task:
